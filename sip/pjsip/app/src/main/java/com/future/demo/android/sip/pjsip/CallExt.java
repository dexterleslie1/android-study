...
public class CallExt extends Call {
    ...
    public void onCallState(OnCallStateParam prm) {
        try {
            CallInfo ci = getInfo();
            int state = ci.getState();

            if (state == pjsip_inv_state.PJSIP_INV_STATE_DISCONNECTED) {
                int statusCode = ci.getLastStatusCode();
                String text= "("+ statusCode +")"+PjsipStatusConstant.getCallStatusTextByStatusCode(statusCode, ci);
                final String message = "通话结束，原因：" + text;
                // 正常通话结束，被叫挂状态码200、主叫挂603
                Log.i(TAG,message+",当前状态："+statusCode);
                ...
                int statusCode = ci.getLastStatusCode();
                // 状态码200
                Log.i(TAG,"被叫接听电话正在连接中,当前状态："+statusCode);
            } else if(state == pjsip_inv_state.PJSIP_INV_STATE_CONFIRMED) {
                int statusCode = ci.getLastStatusCode();
                Log.i(TAG,"已建立通话连接，可以开始说话,当前状态："+statusCode);
                stopRingtone();
                // 已建立通话连接，可以开始说话
                ...
                int statusCode = ci.getLastStatusCode();
                // 状态码183
                Log.i(TAG, "已拨通被叫电话，等待被叫接听,当前状态："+statusCode);
